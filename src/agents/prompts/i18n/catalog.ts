...
const catalogByLocale: Record<PromptLocale, PromptCatalog> = {
  zh: {
    ...
    workerTask: {
      ...
      knowledgeGraphAction: '执行时沿已有事实继续推进。写入公共记忆时优先输出下方 ```graph_* 结构化代码块（运行时自动物化到黑板）。需要即时读写图时调用 blackboard 工具，action 取 write_fact / declare_intent / add_edge / supersede_node / read_graph。代码块示例：',
      ...
      teamCommunicationRules: [
        ...
        '跨栈接口/schema/props/env/命令契约发生新增或变化时，最终输出包含 ```graph_contract 代码块（surface/title/content 等字段，运行时写入黑板）；重要方案沉淀用 ```graph_design_doc 代码块',
    ...
    judges: {
      ...
      eternalPatrol: {
        ...
        system: [
          ...
          '当没有 eternal_goal、fingerprint 未变化、last_patrol_outcome != productive 且 has_open_work=false 时，优先选择 skip 退避待命（idle 退避会自动拉长 patrol 间隔，不烧 tokens），等外部事件（worker 完成、文件变化、fingerprint delta）再恢复 patrol。',
          '除非用户显式需要人类输入（凭据、授权决策等仅靠人类才能提供的信息），否则不要 yield_user；eternal mode 的契约是持续自主看护，不主动把控制权交还用户。',
          '不要仅因为一轮 idle 或 fingerprint 未变化，就对 active eternal_goal 返回 yield_user 或 skip；active goal 应持续 patrol 推进。',
  ...
  en: {
    ...
    workerTask: {
      ...
      knowledgeGraphAction: 'Continue from existing facts. To write public memory, prefer the ```graph_* fenced code blocks below (runtime materializes them onto the blackboard). For interactive graph I/O, call the blackboard tool with action write_fact / declare_intent / add_edge / supersede_node / read_graph. Code block examples:',
      ...
      teamCommunicationRules: [
        ...
        'When cross-stack interface/schema/props/env/command contracts are added or changed, final output includes a ```graph_contract code block (fields surface/title/content; runtime writes it to the blackboard); use a ```graph_design_doc code block for important design decisions',
    ...
    judges: {
      ...
      eternalPatrol: {
        ...
        system: [
          ...
          'Prefer skip (not yield_user) when there is no eternal_goal AND fingerprint did NOT change AND last_patrol_outcome != productive AND has_open_work=false. In eternal mode, back off with skip and stay on standby; the idle backoff stretches the interval so a standing skip does not waste tokens, and any external event resumes patrol.',
          'Do not yield_user on your own. Eternal mode means staying autonomous: only yield_user when continuing is genuinely impossible without human-only input (e.g. missing credentials the user must supply). A merely idle or unchanged round is NOT such a case.',

  ...
  zh: {
    ...
    leader: {
      ...
      eternalGoal: {
        ...
        completedToolResult: (description) => `Eternal 目标「${description}」已标记为完成。Leader 进入待命，可设置新 goal 继续。`,
        ...
        toolDescription: '标记当前 Eternal 目标为已完成。仅当有充分证据证明目标已达成时调用。调用后 goal 被清除，Leader 进入 idle 待命（用户可设置新 goal 继续）。不要在目标仍有未验证部分时调用。',
  ...
  en: {
    ...
    leader: {
      ...
      eternalGoal: {
        ...
        completedToolResult: (description) => `Eternal goal "${description}" marked as complete. Leader is standing by — set a new goal to continue.`,
        ...
        toolDescription: 'Mark the current Eternal goal as complete. Only call when there is concrete evidence the goal has been achieved. After calling, the goal is cleared and Leader enters idle standby (user can set a new goal to continue). Do not call while parts of the goal are still unverified.',

...
import { playInitIntro, renderInitNotice, renderInitStep } from './cli_init_banner.js';
...
import { detectCommonModelProviderFromBaseUrl, getCommonModels } from './cli_model_suggestions.js';
import {
...
async function interactiveInit(): Promise<void> {
  ...
  // 选择 Provider
  console.log(renderInitStep(1, 3, t('cli.provider_prompt')));
  console.log(t('cli.provider_option_openai'));
  console.log(t('cli.provider_option_anthropic'));
  console.log(t('cli.provider_option_auto'));

  const { providerChoice } = await inquirer.prompt([{
    type: 'input',
    name: 'providerChoice',
    message: t('cli.provider_choice_prompt'),
    default: '3',
  }]);

  const providerMap: Record<string, string> = { '1': 'openai', '2': 'anthropic', '3': 'auto' };
  const selectedProvider = providerMap[providerChoice] || 'auto';

  const userSettings = { ...config };
  userSettings.llm = { ...config.llm };
  userSettings.llm.provider = selectedProvider as 'openai' | 'anthropic' | 'auto';

  // v3: 收集 apiKey + baseUrl，写入 model_providers + credentials
  let resolvedBaseUrl = '';
  let initApiKey = '';

  console.log(renderInitStep(2, 3, t('cli.api_key_step')));
  const defaultBaseUrl = selectedProvider === 'anthropic' ? 'https://api.anthropic.com' : 'https://api.openai.com/v1';
  const { apiKey, baseUrl } = await inquirer.prompt([
    { type: 'password', name: 'apiKey', message: t('cli.api_key_prompt'), default: '' },
    { type: 'input', name: 'baseUrl', message: t('cli.base_url_prompt'), default: defaultBaseUrl },
  ]);

  if (apiKey) initApiKey = apiKey;
  if (baseUrl) {
    resolvedBaseUrl = baseUrl;
    if (detectCommonModelProviderFromBaseUrl(baseUrl) === 'anthropic') {
      console.log(chalk.dim(`${t('cli.detected_provider')} ANTHROPIC`));
    }
  }

  // ── 步骤 3：模型选择 ──────────────────────────────────────────────
  console.log(renderInitStep(3, 3, t('cli.model_select_step')));
  console.log(chalk.dim(t('cli.model_select_hint')));

  const commonModels = getCommonModels(selectedProvider, resolvedBaseUrl);
  const CUSTOM_KEY = t('cli.model_custom_input');
  const leaderChoices = [...commonModels, new inquirer.Separator(), CUSTOM_KEY];

  const { leaderChoice } = await inquirer.prompt([{
    type: 'list',
    name: 'leaderChoice',
    message: `${t('cli.model_leader_prompt')}:`,
    choices: leaderChoices,
    default: commonModels[0],
  }]);

  let leaderModel: string = leaderChoice;
  if (leaderChoice === CUSTOM_KEY) {
    const { customLeader } = await inquirer.prompt([{
      type: 'input',
      name: 'customLeader',
      message: `${t('cli.model_leader_prompt')}:`,
      default: commonModels[0],
    }]);
    leaderModel = (customLeader as string).trim() || commonModels[0];
  }
  userSettings.llm.leader_model = leaderModel;
  console.log(chalk.green(t('cli.model_selected', leaderModel)));

  // Agent 模型（默认与 Leader 相同）
  const { useDiffAgent } = await inquirer.prompt([{
    type: 'confirm',
    name: 'useDiffAgent',
    message: t('cli.model_diff_agent_prompt'),
    default: false,
  }]);

  if (useDiffAgent) {
    const agentChoices = [...commonModels, new inquirer.Separator(), CUSTOM_KEY];
    const { agentChoice } = await inquirer.prompt([{
      type: 'list',
      name: 'agentChoice',
      message: `${t('cli.model_agent_prompt')}:`,
      choices: agentChoices,
      default: leaderModel,
    }]);

    let agentModel: string = agentChoice;
    if (agentChoice === CUSTOM_KEY) {
      ...
      agentModel = (customAgent as string).trim() || leaderModel;
    }
    userSettings.llm.agent_model = agentModel;
    console.log(chalk.green(t('cli.model_selected', agentModel)));
  } else {
    userSettings.llm.agent_model = leaderModel;
    console.log(chalk.dim(`  Agent: ${t('cli.model_same_as_leader')} (${leaderModel})`));
  }

  // v3: 写入 model_providers + credentials
  if (initApiKey) {
    const envKeyName = `INIT_API_KEY_${leaderModel.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase()}`;
    // 根据 URL 判断 provider 类型
    const providerType = (resolvedBaseUrl && resolvedBaseUrl.toLowerCase().includes('anthropic'))
      ? 'anthropic'
      : 'openai';

    // 构造 model_providers 条目（leaderModel 和 agentModel 可能相同，去重）
    const modelIds = Array.from(new Set([leaderModel, userSettings.llm.agent_model]));
    const providerModels: import('./config.js').ModelProviderConfig[] = modelIds.map(modelId => ({
      id: modelId,
      name: modelId,
      provider: providerType,
      envKey: envKeyName,
      apiKey: '',  // resolved at runtime from credentials[envKeyName]
      baseUrl: resolvedBaseUrl || '',
    }));

    userSettings.llm.model_providers = {
      ...userSettings.llm.model_providers,
  ...
  try {
    ...
    console.log(chalk.green.bold(`${t('cli.config_saved')} ${SETTINGS_FILE}`));

    // 显示最终配置
    console.log(chalk.bold(t('cli.current_config')));
    console.log(`  Provider: ${userSettings.llm.provider}`);
    console.log(`  Leader Model: ${userSettings.llm.leader_model}`);
    console.log(`  Agent Model: ${userSettings.llm.agent_model}`);
    if (resolvedBaseUrl) console.log(`  Base URL: ${resolvedBaseUrl}`);
    if (initApiKey) console.log(`  API Key: ${'*'.repeat(8)}...`);
    console.log();
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error(chalk.red.bold(`${t('cli.config_save_failed')} ${msg}`));
    console.error(chalk.dim(`${t('cli.check_permissions_hint')} ${SETTINGS_FILE}`));
  }
}
...
async function startTUI(sessionId?: string): Promise<void> {
  ...
  if (!checkInitialized()) {
    ...
    const bootstrapReport = runBootstrapDoctor({ workspace: process.cwd(), repair: true });
    const blockingChecks = bootstrapReport.checks.filter((check) => check.status === 'error');
    if (blockingChecks.length > 0) {
      console.log(formatBootstrapReport(bootstrapReport));
      throw new Error(blockingChecks.map((check) => `${check.name}: ${check.message}`).join('; '));
    }
  ...
  try {
    ...
    // 随机端口：random_port=true 时传 port:0 让 OS 分配
    const useRandomPort = getConfigValue('server.random_port') === true;
    const requestedPort = useRandomPort ? 0 : webPort;
    let actualPort = requestedPort;
    try {

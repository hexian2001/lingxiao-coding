...
export function mergeAgentStatus(currentStatus: unknown, incomingStatus: unknown): NormalizedAgentStatus {
  ...
  // running 不允许降级到 idle：中间状态字符串（"⏳ LLM retry..."等）normalizeAgentStatus
  // 无法识别时会 fallback 为 idle，但 agent 实际仍在运行；允许降级会导致 UI 闪烁。
...
export type RuntimeWaitGateKind = 'permission' | 'review' | 'waiting' | 'ask_user' | 'idle';

export interface RuntimeWaitGate {
  kind: RuntimeWaitGateKind;
  source: 'system' | 'user' | 'leader' | 'worker' | 'unknown';
}
 ...
 * the explicit backend facts; runningWorkers is the detail list and only acts
 * as a compatibility fallback when explicit facts are absent.
 */
...
export function deriveRuntimeWaitGate(runtimeState: unknown): RuntimeWaitGate | null {
  ...
  const pendingUserInput = state.pendingUserInput && typeof state.pendingUserInput === 'object'
    ...
    : {};
  const pendingUserGate = state.pendingUserGate && typeof state.pendingUserGate === 'object'
    ? state.pendingUserGate as Record<string, unknown>
    : {};
  const pendingKind = String(pendingUserInput.kind || '').trim().toLowerCase();
  const gateKind = String(pendingUserGate.kind || '').trim().toLowerCase();
  ...
  if (pendingPermissionRequest || gateKind === 'permission' || pendingKind === 'permission_request') {
  ...
  if (leader.pendingReview || gateKind === 'plan_review' || pendingKind === 'plan_review') {
    return { kind: 'review', source: 'user' };
  }

  if (gateKind === 'ask_user') {
    return { kind: 'ask_user', source: 'leader' };
  }

  if (gateKind === 'idle_wait') {
    return { kind: 'idle', source: 'system' };
  }
